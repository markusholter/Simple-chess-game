...
from chess_game.objects.pieces.Pawn import Pawn
from chess_game.objects.pieces.King import King
from chess_game.objects.pieces.Queen import Queen
from chess_game.objects.pieces.Bishop import Bishop
from chess_game.objects.pieces.Knight import Knight
...
class Board:
    ...
    def make_board(self):
        ...
        return [
            [
                ("cell black", piece_positions.get(i, [None] * 8)[j], f"{i} {j}")
                if (i + j) % 2 == 1
    ...
    def turn(self, move: str, white: bool, fromUser=True):
        ...
        if isinstance(piece, King):
            if piece.getWhite():
                self.whiteKing = tuple(end)
            else:
                self.blackKing = tuple(end)

        newBoard = self.makeNewBoard(start, end, piece, self.board)
        if isinstance(piece, Pawn) and piece.getEnpassant():
            newBoard[start[0]][end[1]] = (
                newBoard[start[0]][end[1]][0],
                None,
                newBoard[start[0]][end[1]][2],
            )
        ...
        if check:
            if (
                (white and "w" in check)
                or (not white and "b" in check)
        ...
        if fromUser:
            ...
            if isinstance(piece, Pawn) and piece.getEnpassant():
                piece.removeEnpassent()
            for row in self.board:
                for cell in row:
                    possiblePawn = cell[1]
                    if isinstance(possiblePawn, Pawn) and possiblePawn != piece:
                        possiblePawn.removeDoubleMove()
    ...
    def checkStale(self, white):
        if self.check:
            return False
        possibleMoves = [
            (x, y) for x in [-1, 0, 1] for y in [-1, 0, 1] if x != 0 and y != 0
        ]
        possibleMoves.extend(
            [(x, y) for x in [-2, -1, 1, 2] for y in [-2, -1, 1, 2] if abs(x + y) == 3]
        )
                ...
                for x, y in possibleMoves:
                    ...
                    if self.turn(f"{i} {j},{end[0]} {end[1]}", white, False):
                        current_app.logger.info(
                            f"{piece} Can move from {i} {j} to {end}"
                        )
                        return False
            ...
            for j, cell in enumerate(row):
                ...
                if blockable and not isinstance(piece, King):
                    for pos in possibleBlocks:
                        if piece.turn([i, j], pos, self.board):
                            current_app.logger.info(
                                f"{piece} at position {i, j} can block at position {pos}"
                            )
                            return False
                ...
                if isinstance(piece, King):
                    possibleMoves = [
                        ...
                        and i + x >= 0
                        and j + y >= 0
                        and i + x < len(self.board)
                        and j + y < len(row)
                    ]
                    for pos in possibleMoves:
                        if self.turn(f"{i} {j},{pos[0]} {pos[1]}", white, False):
                            current_app.logger.info(
                                f"{piece} can move from position {i, j} to position {pos} to escape check"
                            )
                            return False
    ...
    def checkCheck(self, newBoard, log=True):
        ...
        for vertical in [-1, 0, 1]:
            for horizontal in [-1, 0, 1]:
                ...
                whiteCheck = self.checkKing(
                    self.whiteKing, True, vertical, horizontal, newBoard, log
                )
                blackCheck = self.checkKing(
                    self.blackKing, False, vertical, horizontal, newBoard, log
                )
                if whiteCheck:
                    check["w"].add(whiteCheck)
                if blackCheck:
                    check["b"].add(blackCheck)
        ...
        if whiteKnight:
            check["w"].add(whiteKnight)
        if blackKnight:
            check["b"].add(blackKnight)
    ...
    def checkKing(self, king, white, vertical, horizontal, newBoard, log):
        ...
        while (
            row < len(newBoard) and row >= 0 and col < len(newBoard[row]) and col >= 0
        ):
            ...
            if piece.canTake(white, vertical, horizontal, distance):
                if log:
                    current_app.logger.info(
                        f"Check by {type(piece)} detected from position {row} {col} to position {king}"
                    )
                return row, col
            return None
    ...
    def checkKnight(self, king, white, newBoard, log):
        for vertical in [-2, -1, 1, 2]:
            for horizontal in [-2, -1, 1, 2]:
                ...
                row = king[0] + vertical
                col = king[1] + horizontal
                if row >= len(newBoard) or row < 0:
                    continue
                if col >= len(newBoard[row]) or col < 0:
                    continue
