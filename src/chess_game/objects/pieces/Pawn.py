...
class Pawn(Piece):
    ...
    def removeDoubleMove(self):
        self.doubleMove = False

    def getEnpassant(self):
        return self.enpassant

    def removeEnpassent(self):
    ...
    def turn(self, start, end, board):
        ...
        if (
            start[1] == end[1]
            and (end[0] - start[0]) * self.direction <= self.getDistance(start)
            and (end[0] - start[0]) * self.direction > 0
            and not endpiece
            and self.checkObstacle(start, end, board, self.direction, 0)
        ):
        ...
        # Rules for using pawn to attack other pieces
            ...
            (start[1] == end[1] + 1 or start[1] == end[1] - 1)
            and (end[0] - start[0]) * self.direction <= 1
            and (end[0] - start[0]) * self.direction > 0
            and (endpiece or self.checkEnemyDoubleMove(start, end, board))
        ):
            if self.checkEnemyDoubleMove(start, end, board):
                self.enpassant = True
            return True
    ...
    def checkEnemyDoubleMove(self, start, end, board):
        ...
        if isinstance(otherPiece, Pawn) and otherPiece.doubleMove:
            return True
        return False
