...
def test_bishop_diagonal_move():
    ...
    assert bishop.turn(start, end, board.get_white_board()[2]), (
...
def test_bishop_diagonal_move_other_direction():
    ...
    assert bishop.turn(start, end, board.get_white_board()[2]), (
        "Bishop should move diagonally in the other direction."
    )
...
def test_bishop_invalid_move_straight_line():
    ...
    assert not bishop.turn(start, end, board.get_white_board()[2]), (
        "Bishop should not move in a straight line."
    )


def test_bishop_move_with_obstacle_in_path():
    board = Board()
    bishop = board.get_white_board()[2][7][2][
        1
    ]  # White bishop at initial position (7, 2)
    start = [7, 2]
    end = [5, 4]
    # Pawns are already in the way from the initial board setup
    assert not bishop.turn(start, end, board.get_white_board()[2]), (
        "Bishop should not move through other pieces."
    )
...
def test_bishop_move_backwards_diagonally():
    ...
    assert bishop.turn(start, end, board.get_white_board()[2]), (
        "Bishop should be able to move backward diagonally."
    )
...
def test_bishop_move_backwards_diagonally_other_direction():
    ...
    assert bishop.turn(start, end, board.get_white_board()[2]), (
        "Bishop should be able to move backward diagonally in the other direction."
    )
