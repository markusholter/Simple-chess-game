...
def test_knight_L_shape_move_forward():
    board = Board()
    knight = board.get_white_board()[2][7][1][
        1
    ]  # White knight at initial position (7, 1)
    start = [7, 1]
    end = [5, 2]
    assert knight.turn(start, end, board.get_white_board()[2]), (
        "Knight should move in an L-shape (2 forward, 1 sideways)."
    )
...
def test_knight_L_shape_move_sideways():
    ...
    assert knight.turn(start, end, board.get_white_board()[2]), (
        "Knight should move in an L-shape (2 forward, 1 sideways) in the other direction."
    )
...
def test_knight_invalid_move():
    ...
    assert not knight.turn(start, end, board.get_white_board()[2]), (
        "Knight should not move like a rook or a pawn."
    )
...
def test_knight_move_backwards_L_shape():
    ...
    assert knight.turn(start, end, board.get_white_board()[2]), (
        "Knight should be able to move backwards in an L-shape."
    )
...
def test_knight_move_backwards_other_direction():
    ...
    assert knight.turn(start, end, board.get_white_board()[2]), (
        "Knight should be able to move backwards in an L-shape in the other direction."
    )
